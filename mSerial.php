...
$sql = 'SELECT * FROM estudiante WHERE serial<>"" AND cedula='. $_POST["cedula"] . ' AND nacionalidad='. $_POST['nacionalidad'];
			...
			while ($row = mysql_fetch_array($query)){
					...
					echo '<tr>
							  ...
							  <td>'. $telefono .'</td>
							  <td>'. $colegio .'</td>
							  <td><form action="mSerial2.php" method="post"><input type="hidden" name="id" value="'. $id .'" /><button class="button secondary" name="acciones" value="mSerial">Serial</button></form></td>
						</tr>
							';
			}
		mysql_close();
