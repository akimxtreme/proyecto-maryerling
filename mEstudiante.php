...
$sql = 'SELECT * FROM estudiante WHERE cedula='. $_POST["cedula"] . ' AND nacionalidad='. $_POST['nacionalidad'];
			...
			while ($row = mysql_fetch_array($query)){
					...
					echo '<tr>
							  ...
							  <td>'. $telefono .'</td>
							  <td>'. $colegio .'</td>
							  <td><form action="mEstudiante2.php" method="post"><input type="hidden" name="id" value="'. $id .'" /><button class="button secondary" name="acciones" value="mEstudiante">Modificar</button></form></td>
						</tr>
							';
			}
		mysql_close();
