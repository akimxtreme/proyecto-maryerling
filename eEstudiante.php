...
$sql = 'SELECT * FROM estudiante WHERE cedula='. $_POST["cedula"] . ' AND nacionalidad='. $_POST['nacionalidad'];
			...
			while ($row = mysql_fetch_array($query)){
					...
					echo '<tr>
							  ...
							  <td>'. $telefono .'</td>
							  <td>'. $colegio .'</td>
							  <td><form action="eEstudiante2.php" method="post"><input type="hidden" name="id" value="'. $id .'" /><button class="button alert secondary" name="acciones" value="eEstudiante">Eliminar</button></form></td>
						</tr>
							';
			}
		mysql_close();
