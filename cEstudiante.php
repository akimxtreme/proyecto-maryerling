...
$sql = 'SELECT * FROM estudiante WHERE cedula='. $_POST["cedula"] . ' AND nacionalidad='. $_POST['nacionalidad'];
			$query = mysql_query($sql,$conexion);
			...
			while ($row = mysql_fetch_array($query)){
					...
					echo '<tr>
							  ...
							  <td>'. $telefono .'</td>
							  <td>'. $colegio .'</td>
							  <td><form action="aEstudianteSuccess.php" method="get"><input type="hidden" name="agregado" value="'. $cedula .'" /><input type="hidden" name="nac" value="'. $cod_nacionalidad .'" /><button class="button success secondary" name="url" value="c">Consultar</button></form></td>
						</tr>
							';
			}
		mysql_close();
...
<?php
// Consultando el Estudiante
$sql = "SELECT * FROM estudiante ORDER BY nombre1";
			$query = mysql_query($sql,$conexion);
			$cont=mysql_num_rows($query);
			while ($row = mysql_fetch_array($query)){
					$id =$row['id'];
