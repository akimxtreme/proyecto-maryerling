...
$sql = 'SELECT * FROM estudiante WHERE serial<>"" AND cedula='. $_POST["cedula"] . ' AND nacionalidad='. $_POST['nacionalidad'];
			...
			while ($row = mysql_fetch_array($query)){
					...
					echo '<tr>
							  ...
							  <td>'. $telefono .'</td>
							  <td>'. $colegio .'</td>
							  <td><form action="aSerialSuccess.php" method="get"><input type="hidden" name="agregado" value="'. $cedula .'" /><input type="hidden" name="nac" value="'. $cod_nacionalidad .'" /><button class="button success secondary" name="url" value="c">Consultar Serial</button></form></td>
						</tr>
							';
			}
		mysql_close();
